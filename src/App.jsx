...
function SummaryTab({entries,compItems,squawks,requiredItems}) {
  const tails=[...new Set(entries.map(e=>e.tail))];
  ...
  return (
    <div>
      {tails.map(tail=>{
        ...
        const anyOverdue = overdue>0 || openSq.length>0 || reqOverdue>0;
        const anyWarn = !anyOverdue && (warning>0 || reqWarn>0);
        const allCurrent = !anyOverdue && !anyWarn;
        return (
          <Card key={tail}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
              <div><div style={{fontSize:11,color:C.muted,textTransform:"uppercase",letterSpacing:".1em",marginBottom:2}}>Aircraft</div><div style={{fontSize:22,fontWeight:600,fontFamily:"'JetBrains Mono',monospace",color:C.accent}}>{tail}</div></div>
              <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
                {overdue>0&&<Badge cls="danger">⚠ {overdue} compliance overdue</Badge>}
                {openSq.length>0&&<Badge cls="danger">⚠ {openSq.length} open squawk{openSq.length>1?"s":""}</Badge>}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:comp.length||openSq.length?12:0}}>
            ...
            {openSq.length>0&&<><div style={{borderTop:"1px solid #991b1b",paddingTop:12,marginBottom:8,fontSize:11,color:C.danger,textTransform:"uppercase",letterSpacing:".08em"}}>Open squawks</div>{openSq.map(s=><div key={s.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13}}>{s.title}</span><Badge cls="danger">OPEN</Badge></div>)}</>}
...
const TABS=[{id:"dispatch",label:"Dispatch"},{id:"times",label:"Times"},{id:"compliance",label:"Compliance"},{id:"summary",label:"Summary"},{id:"profile",label:"Profile"}];
...
export default function AircraftLog() {
  ...
  const overdueReq = [...new Set(entries.map(e=>e.tail))].flatMap(tail=>requiredInspectionsStatus(requiredItems,tail).items.filter(i=>i.status.cls==="danger"));
  const alert       = overdueComp.length>0||openSquawks.length>0||overdueReq.length>0;
    ...
    <>
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"16px 12px 60px"}}>
        {alert&&tab!=="compliance"&&(
          <div style={{background:"#2d0a0a",border:"1px solid #991b1b",borderRadius:8,padding:"10px 14px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{color:C.danger,fontSize:13,fontWeight:500}}>
              {[overdueComp.length>0&&`${overdueComp.length} compliance item${overdueComp.length>1?"s":""} overdue`,openSquawks.length>0&&`${openSquawks.length} open squawk${openSquawks.length>1?"s":""}`,overdueReq.length>0&&`${overdueReq.length} required inspection${overdueReq.length>1?"s":""} overdue/missing`].filter(Boolean).join(" · ")}
            </span>
            <Btn size="sm" variant="danger" onClick={()=>setTab("compliance")}>View →</Btn>
          </div>
        )}
        {tab==="dispatch"  &&<DispatchTab   entries={entries} dispatch={dispatch} setDispatch={setDispatch} compItems={compItems} squawks={squawks} requiredItems={requiredItems} toast={toast}/>}
        ...
        {tab==="summary"   &&<SummaryTab    entries={entries} compItems={compItems} squawks={squawks} requiredItems={requiredItems}/>}
